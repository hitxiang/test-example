...
public class PalindromeGameTest {
  ...
  public void testMultipleUser_multithread() {
    ...
    final CyclicBarrier barrier = new CyclicBarrier(NUM_IN_TOP_RANK * 5 + 1);
    final CountDownLatch endSignal = new CountDownLatch(NUM_IN_TOP_RANK * 5 * 2000);
    ...
    User u;
    AbstractQueue<Score> highestScoreQueue = new PriorityQueue<Score>(NUM_IN_TOP_RANK);
    AbstractQueue<Score> totalScoreQueue = new PriorityQueue<Score>(NUM_IN_TOP_RANK);

    // Now retrieve the result
    for (Future<User> future : allList) {
      try {
        u = future.get();
        highestScoreQueue.add(new Score(Score.TYPE.HIGH, u.getId(), u.getName(), u
            .getHighestScore()));
  ...
  private User processOneUserWithoutRegister() {
    int[] scores =
        new int[] {rand.nextInt(200), rand.nextInt(100), rand.nextInt(200), rand.nextInt(300),
            rand.nextInt(30), rand.nextInt(20), rand.nextInt(25)};
  ...
  private User processOneUser() {
    ...
    int[] scores =
        new int[] {rand.nextInt(10) + rand.nextInt(20), rand.nextInt(20) + rand.nextInt(30),
            rand.nextInt(30) + rand.nextInt(40), rand.nextInt(40) + rand.nextInt(50),
            rand.nextInt(5) + rand.nextInt(15), rand.nextInt(100) + rand.nextInt(5),
